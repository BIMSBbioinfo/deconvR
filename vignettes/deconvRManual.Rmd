...
The `WGBS_data` parameter can be either a **GRanges** object or **methylKit**
object (methylRaw, methylBase, methylRawDB, or methylBaseDB) of CpG locations
and their methylation values.
...
example **GRanges** object `WGBS_GRanges.RDS` in the `inst` folder of the
package (more information on this file can be found below).
...
`simulateCellMix` and `findSignatures` don't suppor other data type than WGBS
yet. However,`deconvolute` can be used to deconvolute any kind of RNA-seq data. 
Here, we will use RNA-seq example data from the **granulator** package. We will
only use small sample of data. Notice that we set IDs column. In order to run
`deconvolute` function, `IDs` column is necessary.

``` {r }
library(granulator)
#To load the data from granulator package
granulator::load_ABIS()
