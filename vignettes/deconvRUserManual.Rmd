---
title: "User Guide for deconvR package"
author: 
...
vignette: >
  %\VignetteIndexEntry{User Guide for deconvR package}
...
origin cell-type proportions of the samples.
These RMSE values very closely match the previous RMSE values. This is to be
expected, since the accuracy of model-predicted methylation values will change
with differing accuracies of model coefficients, which were used to infer origin
cell-type proportions. All models had a satisfactorily low RMSE. While NNLS,
RLM, and QP had much lower RMSE values than SVR, the RMSE of SVR was still low
enough that predictions were close to accurate.
...
These values were calculated by running the model on a bulk sample with
continually decreasing cellular proportions. The limit of detection was inferred
to be the point at which the model thereafter failed to detect the origin cell
type.Once again, limit of detection closely follows model accuracy (as measured
by RMSE).
...
than 10 seconds for 1000 samples. The runtime for RLM was also satisfactory,
taking roughly 2 minutes for 1000 samples. SVR is significantly slower than the
other models. While parallelization helped significantly to decrease runtime, it
still takes roughly 15 minutes for 1000 samples.
