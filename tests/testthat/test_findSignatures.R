...
test_that("findSignatures", {
  ...
  exampleMeta <- data.table(
    "Experiment_accession" = c(colnames(exampleSamples)[-1], colnames(exampleReference)[-1], as.character(1:200)),
    "Biosample_term_name" = c(
      rep("this", 2), rep("that", 13), rep("else", 25), rep(colnames(exampleReference)[2], 1),
