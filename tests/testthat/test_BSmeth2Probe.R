...
test_that("BSmeth2Probe", {
    ...
    expect_equal(NCOL(results), 1 + length(colnames(GenomicRanges::mcols(WGBS_GRanges))))
    expect_equal(colnames(results), c("IDs", colnames(GenomicRanges::mcols(WGBS_GRanges))))
    ...
    expect_error(NROW(BSmeth2Probe(probe_id_locations = probe_ids[0], WGBS_data = WGBS_GRanges)))
    expect_error(NROW(BSmeth2Probe(probe_id_locations = probe_ids, WGBS_data = WGBS_GRanges[0])))

    expect_lt(NROW(results), NROW(BSmeth2Probe(probe_id_locations = probe_ids, WGBS_data = WGBS_GRanges, cutoff = 100)))
    expect_gt(NROW(results), NROW(BSmeth2Probe(probe_id_locations = probe_ids, WGBS_data = WGBS_GRanges, cutoff = 0)))
    expect_lt(NROW(results), NROW(BSmeth2Probe(probe_id_locations = probe_ids, WGBS_data = WGBS_GRanges, multipleMapping = TRUE)))
    expect_gt(NROW(results), NROW(BSmeth2Probe(probe_id_locations = probe_ids[1:NROW(probe_ids) / 2], WGBS_data = WGBS_GRanges)))
    expect_gt(NROW(results), NROW(BSmeth2Probe(probe_id_locations = probe_ids, WGBS_data = WGBS_GRanges[1:NROW(WGBS_GRanges) / 2])))

    expect_equal(results, BSmeth2Probe(probe_id_locations = probe_ids_df, WGBS_data = WGBS_GRanges))
    expect_equal(
    ...
    expect_error(BSmeth2Probe(probe_id_locations = probe_ids, WGBS_data = WGBS_GRanges, cutoff = -1))
    expect_error(BSmeth2Probe(probe_id_locations = probe_ids, WGBS_data = probe_ids))
    expect_error(BSmeth2Probe(probe_id_locations = WGBS_GRanges, WGBS_data = WGBS_GRanges))
    expect_error(BSmeth2Probe(probe_id_locations = NULL, WGBS_data = WGBS_GRanges))
