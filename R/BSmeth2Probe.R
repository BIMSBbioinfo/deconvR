...
BSmeth2Probe <- function(probe_id_locations, WGBS_data, cutoff = 10, multipleMapping = FALSE) {
  ...
  if (cutoff > 0) { # only need to do "nearlyOverlaps" if cutoff > 0
    ...
    if (nrow(nearlyOverlaps_df) > 0) {
      ...
      if (!multipleMapping) {
        if (nrow(overlaps_df) > 0) {  # remove where CpG already mapped in first round if multipleMapping has been set to false
          nearlyOverlaps_df <- subset(nearlyOverlaps_df,
                                      !(is.element(data.table::transpose(BiocGenerics::as.data.frame(nearlyOverlaps_df$WGBS_data)),
