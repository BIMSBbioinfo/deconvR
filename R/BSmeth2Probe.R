...
#' @param probe_id_locations Either a dataframe or GRanges object containing probe IDs and their locations. Must contain columns named "ID", "CHR", "Start", "End", and "Strand". Start and end locations should be 1-based coordinates. Note that any row with NA values will not be used. Example dataframe illumina_probes_hg38_GRanges.RDS in inst folder included in package.
#' @param WGBS_data Either a GRanges object or methylKit object (methylRaw, methylBase, methylRawDB, or methylBaseDB) of CpG locations and their methylation values. Contains locations ("seqnames", "ranges", "strand") and metadata columns "coverage", "numCs", and "numTs".
...
#' @return A dataframe with two columns: "ID" and "methylation"
#' ID for each probe which was mapped, and methylation is the methylation value of the WGBS CpG to which it was matched (where either it overlapped or the gap was < cutoff).
...
BSmeth2Probe = function(probe_id_locations, WGBS_data, cutoff = 10, multipleMapping = FALSE) {
  ...
  allresults = rbind(overlaps_df, nearlyOverlaps_df) #combine ll of the mapping into one dataframe
  allresults = allresults[,c("ID", "methylation")]
  if (nrow(allresults) > 0) {
    allresults = stats::aggregate(x = allresults$methylation, by = list(ID = allresults$ID), FUN = mean) #if a probe was mapped to multiple CpGs, take the mean methylation value
    colnames(allresults) = c("ID", "methylation")
  }
  if (nrow(allresults) == 0) {print("Result dataframe is empty. No matches could be found.")}

  allresults #the return value is a dataframe with 2 columns: ID and methylation
