...
BSmeth2Probe <- function(probe_id_locations, WGBS_data, cutoff = 10, multipleMapping = FALSE) {
    ...
    if (class(probe_id_locations) == "data.frame") {
        if (any(is.null(probe_id_locations$CHR), is.null(probe_id_locations$Start), is.null(probe_id_locations$End),
            is.null(probe_id_locations$Strand), is.null(probe_id_locations$ID))) {
        ...
        # if the probe_id_locations is a dataframe, turn it into GRanges before continuing
        probe_id_locations <- GenomicRanges::GRanges(seqnames = probe_id_locations[, "CHR"], ranges = IRanges::IRanges(start = probe_id_locations[,
            "Start"], end = probe_id_locations[, "End"]), strand = probe_id_locations[, "Strand"], ID = probe_id_locations[,
            "ID"])
    ...
    if (class(WGBS_data) == "GRanges") {
        ...
        S4Vectors::elementMetadata(WGBS_data) <- data.table::nafill(as.data.frame(S4Vectors::elementMetadata(WGBS_data)),
            fill = 0)
        ...
        if ((any(as.data.frame(S4Vectors::elementMetadata(WGBS_data)) < 0)) || (any(as.data.frame(S4Vectors::elementMetadata(WGBS_data)) >
            1))) {
            stop("WGBS_data metadata columns should be methylation values of sample(s), between 0 and 1")
        }
    }
    ...
    if (cutoff > 0) {
        ...
        if (nrow(nearlyOverlaps_df) > 0) {
            nearlyOverlaps_df <- cbind(nearlyOverlaps_df, distance = IRanges::distance(nearlyOverlaps_df$WGBS_data,
                nearlyOverlaps_df$probe_id_locations))  #the distance of the gap between probe location and WGBS data location
            ...
            if (!multipleMapping) {
                if (nrow(overlaps_df) > 0) {
                  ## remove where CpG already mapped in first round if multipleMapping has been set to false
                  nearlyOverlaps_df <- subset(nearlyOverlaps_df, !(is.element(data.table::transpose(BiocGenerics::as.data.frame(nearlyOverlaps_df$WGBS_data)),
                    data.table::transpose(BiocGenerics::as.data.frame(overlaps_df$WGBS_data)))))
                }
