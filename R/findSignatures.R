...
#' @param sampleMeta dataframe, must have first column for accession ID of each sample, and second column for cell type of sample, rows are samples
...
findSignatures <- function(samples, sampleMeta, atlas=NULL, variation_cutoff = NULL) {
  ...
  for (sample_id in colnames(samples)[-1]) {
    assertthat::assert_that(sample_id %in% unlist(sampleMeta[,1]), msg = "All column names of samples (other than IDs column) should be present in sampleMeta Experiment_accession")
  }
  ...
  if(is.null(atlas)) {
    ...
    for (celltype in colnames(atlas)[-1]) {                    # looking at all cell types in the atlas
        ...
        sampleMeta  <- rbind(sampleMeta, list(celltype, celltype)) # if the cell type isn't in metadata, just add a row saying it maps to itself
        }
      }
    extendedAtlas <- data.table::merge.data.table(atlas,samples,by = "IDs", sort = FALSE) # merge atlas with samples
  }
  rowsToDelete <- c()
  ...
  for (tissue in unlist(unique(sampleMeta[,2]))) {
      ...
      if (length(accession_ids) > 0) { # if no accession IDs map to this cell type then no need to continue
        ...
        if (!(is.null(variation_cutoff))) {
          ...
          tissue_variance <- extendedAtlas[,matrixStats::rowVars(as.matrix(.SD)), .SDcols = unlist(accession_ids)]
        ...
        pooled_column <- rowMeans(extendedAtlas[,.SD, .SDcols = unlist(accession_ids), drop = FALSE]) # pool samples for same tissue
        extendedAtlas[,as.character(unlist(accession_ids)) := NULL ] # remove single samples
