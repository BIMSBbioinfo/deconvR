...
deconvolute <- function(reference=readRDS(system.file("reference_atlas_nodup.RDS", package = "deconvR")), vec = NULL, bulk, model="nnls") {
  ...
  find_partial_rsq <- function(observed, predicted, ref, vec) {
    if (is.null(vec)) {
      ...
      assertthat::assert_that(length(vec) == length(predicted), msg = paste("vector should be length", length(predicted), "but is length", length(vec)))
    }
  ...
  if (model == "nnls") {    #non negative least squares
    oper <- foreach::foreach(h = seq(2, ncol(bulk)), .inorder = TRUE, .combine = "comb", .multicombine=TRUE, .init=list(c(), list())) %dopar% { # it's implemented this way to work with doParallel
    ...
    oper <- foreach::foreach(h = seq(2, ncol(bulk)), .inorder = TRUE, .combine = "comb", .multicombine=TRUE, .init=list(c(), list())) %dopar% {
      thedata <- tidyr::drop_na(merge(dplyr::select(bulk, 1, h), reference, by="IDs"))[,-1] # merge each sample to reference table, removing IDs that aren't present in both
      mix <- as.matrix(thedata[,1])   # first column is the bulk mixed sample
      ref <- as.matrix(thedata[,-1])  # the rest of columns come from reference
      model <- e1071::best.tune("svm", train.x = mix ~ ref, kernel="linear", type="nu-regression", scale=FALSE, ranges = list(nu = seq(0.25,0.5,0.75))) # find the best nu values for the models
