#' A function to deconvolute of bulk samples to their origin proportions using data from reference atlas (e.g. methylation signatures)
...
#' @param vec The user may provide a vector with which partial R-squared of the results will be calculated.
#' The length must match the number of rows of the reference and bulk tables merged on the ID column (with NAs removed).
...
#' @return A list, first is a dataframe which contains predicted cell-type proportions of bulk methylation profiles in 'bulk', second is a list of partial-rsq values of results, one value per sample
...
deconvolute <- function(reference = readRDS(system.file("reference_atlas_nodup.RDS", package = "deconvR")),
    ...
    results <- data.frame(matrix(ncol = length(colnames(reference)) - 1, nrow = length(colnames(bulk)) - 1,
        dimnames = list(colnames(bulk)[-1], colnames(reference)[-1])))
    h <- 2
    comb <- function(x, ...) {
        lapply(seq_along(x), function(i) c(x[[i]], lapply(list(...), function(y) y[[i]])))
    }

    bulk <- tidyr::drop_na(bulk)  # get rid of rows in both tables with na values
    reference <- tidyr::drop_na(reference)

    find_partial_rsq <- function(observed, predicted, ref, vec) {
        if (is.null(vec)) {
            vec <- rowMeans(ref)  # vector defaults to row means of reference (after removing IDs which weren't in bulk sample)
        } else {
            assertthat::assert_that(length(vec) == length(predicted), msg = paste("vector should be length",
                length(predicted), "but is length", length(vec)))
        }

        rsq_partial <- rsq::rsq.partial(stats::lm(predicted ~ observed + vec), stats::lm(predicted ~ vec))$partial.rsq  # calculate partial r-squared
        return(rsq_partial)
    }

    if (model == "nnls") {
        # non negative least squares it's implemented this way to work with doParallel
        oper <- foreach::foreach(h = seq(2, ncol(bulk)), .inorder = TRUE, .combine = "comb", .multicombine = TRUE,
            .init = list(c(), list())) %dopar% {
            ...
            mix <- as.matrix(thedata[, 1])  # first column is the bulk mixed sample
            ref <- as.matrix(thedata[, -1])  # the rest of columns come from reference
            model <- nnls::nnls(data.matrix(ref), mix)
            coefficients <- model$x
            sumOfCof <- sum(coefficients)
            coefficients <- coefficients/sumOfCof  # normalize so coefficients add to 1

            return(list(find_partial_rsq(mix, ref %*% coefficients, ref, vec), coefficients))
        }
    } else if (model == "svr") {
        ...
        oper <- foreach::foreach(h = seq(2, ncol(bulk)), .inorder = TRUE, .combine = "comb", .multicombine = TRUE,
            ...
            mix <- as.matrix(thedata[, 1])  # first column is the bulk mixed sample
            ref <- as.matrix(thedata[, -1])  # the rest of columns come from reference
            model <- e1071::best.tune("svm", train.x = mix ~ ref, kernel = "linear", type = "nu-regression",
                scale = FALSE, ranges = list(nu = seq(0.25, 0.5, 0.75)))  # find the best nu values for the models
            coefficients <- t(model$coefs) %*% model$SV
            coefficients <- ifelse(coefficients < 0, 0, coefficients)  # normalize so coefficients > 0
            sumOfCof <- sum(coefficients)
            coefficients <- as.numeric(coefficients/sumOfCof)  # normalize so coefficients add to 1
        ...
        # for reference about these values read solve.QP documentation also helpful:
        # https://github.com/dariober/quadratic-programming-deconvolution/blob/master/quadratic_programming_tutorial.pdf
        beq <- c(1)
        ...
        oper <- foreach::foreach(h = seq(2, ncol(bulk)), .inorder = TRUE, .combine = "comb", .multicombine = TRUE,
            ...
            coefficients <- quadprog::solve.QP(Dmat = Dmat, dvec = dvec, Amat = t(Amat), bvec = bvec, meq = meq)$solution

            coefficients <- ifelse(coefficients < 0, 0, coefficients)  # make sure all coefficients are > 0

            sumOfCof <- sum(coefficients)
            coefficients <- coefficients/sumOfCof  # make sure coefficients add to 1
        ...
        oper <- foreach::foreach(h = seq(2, ncol(bulk)), .inorder = TRUE, .combine = "comb", .multicombine = TRUE,
            ...
            model <- suppressWarnings(MASS::rlm(ref, mix, maxit = 100))  # rlm via re-weighted least squares with maximum 100 iterations
            coefficients <- model$coefficients  # (i found 100 iterations the best balence of time vs accurracy but can be changed)

            coefficients <- ifelse(coefficients < 0, 0, coefficients)  # normalize coefficients so all > 0
